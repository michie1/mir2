...
char *html; // Temp variable with latest html buffer
...
{ 
	...
	{             
		...
		if (html_parser_cmp_tag(hsp, "a", 1)) //Compares the tag name and the string “a”.The argument 1 is a string length.Returns 1 for equality otherwise returns 0
			if (html_parser_cmp_attr(hsp, "href", 4))//Compares the attribute name and the string “href”. The argument 4 is a string length.Returns 1 for equality otherwise returns
				...
				{ 
					...
					strcat(hrefs, html_parser_val(hsp));
					strcat(hrefs, "\n");
					//strcat(href, html_parser_val(hsp));
...
static int Download_URL(char * url,HTMLSTREAMPARSER *hsp)
{
	baseurl = url;
	int success = 1;
...
char *GetWebPage(char *myurl) {
	...
	*/
	...
	success = Download_URL(myurl, hsp);
	if(success == 1) {
		return html;
	} else {
		return NULL;
